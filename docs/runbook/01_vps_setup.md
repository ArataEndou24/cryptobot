...
**手元に常時稼働できる Mac がある場合は、この手順書は不要。手順書 00 を使う。**
VPS は、自宅運用のリスク（停電、回線断）が資金規模に見合わなくなった段階で検討する。
...
| リージョン | 欧州（Hetzner なら Falkenstein か Helsinki）。時間軸の関係で距離は成果に影響せず、アジアより大幅に安い |
| 料金 | 欧州ロケーションなら月数百円〜千円強。アジアは同構成で 3 倍近くになる |
| 候補 | Hetzner（推奨: Cost-Optimized タブ、2 vCPU / 4 GB、IPv4 あり）。代替は Vultr、Linode、ConoHa、さくら |

判断基準は「稼働率の実績」と「料金の単純さ」。速度は不要。メモリ 4 GB は下限で、2 GB は避ける。上位プランへの変更は後からできる。
